...
namespace cdk {
  ...
  class postfix_ix86_emitter: public basic_postfix_emitter {
    ...
    std::string _work_segment;  // main work segment
  ...
  public:
    ...
    void DOUBLE(double value) override {
      ...
      auto work_segment = _work_segment;
      RODATA();
      ALIGN();
      LABEL(label);
      SDOUBLE(value);
      // return to work segment
      // note that multiple TEXT segments are possible
      _segment(work_segment);
      ADDR(label);
      LDDOUBLE();
    }
